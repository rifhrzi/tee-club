...
function SimpleLoginContent() {
  ...
  useEffect(() => {
    ...
    if (accessDenied) {
      console.log("SimpleLogin: Access denied parameter detected");
    ...
    if (isAuthenticated && user) {
      ...
      if (redirect.startsWith("/admin") && user.role !== "ADMIN") {
        console.log("SimpleLogin: User is not admin, showing error message");
        setError("You don't have permission to access the admin area");
      } else {
  ...
  return (
    <div className="flex min-h-screen items-center justify-center bg-gray-50 px-4 py-12 sm:px-6 lg:px-8">
      <div className="w-full max-w-md space-y-8">
        <div>
          <h2 className="mt-6 text-center text-3xl font-extrabold text-gray-900">Simple Login</h2>
          <p className="mt-2 text-center text-sm text-gray-600">
            Don't have an account?{" "}
            <Link href="/simple-signup" className="font-medium text-blue-600 hover:text-blue-500">
              Sign up
            </Link>
            {" or "}
          ...
          <form onSubmit={handleSubmit}>
            ...
            <div>
              <button
                ...
                className={`group relative mt-4 flex w-full justify-center rounded-md border border-transparent bg-blue-600 px-4 py-2 text-sm font-medium text-white hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2 ${
